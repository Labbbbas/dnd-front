...
export default function NpcDialog({
  ...
  const saveNpc = async () => {
    if (action === "add") { // If the action is "add"
      try {
        const response = await axios.post("http://api_npc:8004/api/v1/npcs", npcData); // Send data to the server to add the npc
      ...
      try {
        const response = await axios.put(`http://api_npc:8004/api/v1/npcs/${npcData._id}`, npcData); // Update npc on the server
