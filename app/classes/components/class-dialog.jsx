...
export default function ClassDialog({
  ...
  const saveClass = async () => {
    ...
    if (action === "add") { // If the action is "add"
      try {
        const response = await axios.post("https://localhost:8003/api/v1/classes", classDataToSend); // Send data to the server to add the class
      ...
      try {
        const response = await axios.put(`https://localhost:8003/api/v1/classes/${classData._id}`, classDataToSend); // Update class on the server
    ...
    > 
      ...
      <DialogActions sx={{ justifyContent: "center", gap: 2 }}> {/*gap property adds spacing between the buttons*/}
        ...
        <Button 
          ...
          // Event handler to save the class (either adding or editing)
          onClick={saveClass} // Type of the button (submit)
        >
