...
export default function ClassDialog({
  ...
  const saveClass = async () => {
    ...
    if (action === "add") { // If the action is "add"
      try {
        const response = await axios.post("http://api_class:8003/api/v1/classes", classDataToSend); // Send data to the server to add the class
      ...
      try {
        const response = await axios.put(`http://api_class:8003/api/v1/classes/${classData._id}`, classDataToSend); // Update class on the server
